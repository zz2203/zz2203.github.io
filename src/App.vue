...
const showCityBoundaries = async (provinceId: string) => {
  ...
  cityGroup
    ...
    //     .on(
    //       "mouseenter",
    //       debounce(function (this: SVGPathElement, event: any, d: any) {
    //         d3.select(this)
...
const maxStudentCount = Math.max(
  ...Array.from(provinceStats.values()).map((s) => s.count)
);
...
const getProvinceColor = (mapProvinceName: string) => {
  ...
  if (stats && stats.count > 0) {
    console.log(
      `省份映射: ${mapProvinceName} -> ${studentProvinceName}, 学生数: ${stats.count}`
    );
  }
...
const drawMap = async () => {
  ...
  try {
    ...
    provinceGroup
      ...
      //   "mouseenter",
      //   debounce(function (this: SVGPathElement, event: any, d: any) {
      //     d3.select(this).attr("stroke", "#999999").attr("stroke-width", 2);

      //     showEnhancedTooltip(event, d);
      //   }, 300)
      // )
      // .on("mouseleave", function (this: SVGPathElement, event: any, d: any) {
      //   d3.select(this).attr("stroke", "#cccccc").attr("stroke-width", 1.5);

      //   hideTooltip();
      // })
      .on("click", async function (event: any, d: any) {
        const provinceId = d.properties.adcode
          ? d.properties.adcode.toString().substring(0, 2)
          : null;
        ...
        if (provinceId) {
          ...
          const studentProvinceName = mapProvinceNameToStudentData(
            d.properties.name
          );
    ...
    const legendItems = [
      {
        color: "#1565c0",
        label: `${Math.ceil(maxStudentCount * 0.8)}-${maxStudentCount}人`,
        range: "high",
      },
      {
        color: "#1976d2",
        label: `${Math.ceil(maxStudentCount * 0.6)}-${Math.ceil(
          maxStudentCount * 0.8
        )}人`,
        range: "medium-high",
      },
      {
        color: "#42a5f5",
        label: `${Math.ceil(maxStudentCount * 0.4)}-${Math.ceil(
          maxStudentCount * 0.6
        )}人`,
        range: "medium",
      },
      {
        color: "#90caf9",
        label: `${Math.ceil(maxStudentCount * 0.2)}-${Math.ceil(
          maxStudentCount * 0.4
        )}人`,
        range: "low",
      },
      {
        color: "#e3f2fd",
        label: `1-${Math.ceil(maxStudentCount * 0.2)}人`,
        range: "very-low",
      },
...
const showEnhancedTooltip = (event: any, d: any) => {
  ...
  if (stats && stats.count > 0) {
    ...
    if (stats.students.length <= 3) {
      ...
      content += `<div style="font-size: 11px; margin-left: 8px; color: #999999;">...还有${
        stats.count - 2
      }人</div>`;
    }
  } else {
...
const filteredStudents = computed(() => {
  ...
  if (selectedProvince.value) {
    filtered = filtered.filter(
      (s: Student) => s.province === selectedProvince.value
    );
  }
...
const drawCityMarkers = () => {
  ...
  // 获取有学生的城市数据
  const studentsWithCoords = students
    .map((student: Student) => {
  ...
  const cityStats = new Map<
    string,
    { coord: CityCoordinate; students: Student[]; count: number }
  >();
  ...
  Array.from(cityStats.values()).forEach((cityData) => {
    ...
    // 获取当前缩放比例
    const zoomLevel = currentZoomLevel.value;
    const fontSize = Math.min(14 * Math.sqrt(zoomLevel), 10 * zoomLevel) / zoomLevel;
    cityMarkersGroup
      .append("text")
...
const showCityTooltip = (event: any, cityData: any) => {
  ...
  if (cityData.students.length <= 3) {
    ...
    content += `<div style="font-size: 11px; margin-left: 8px; color: #ffab91;">...还有${
      cityData.count - 2
    }人</div>`;
  }
  ...
  if (svg) {
    svg
      ...
      .style("filter", (d: any) => {
        ...
        return studentProvinceName === provinceName
          ? "brightness(1.2) drop-shadow(0 0 8px rgba(255, 87, 34, 0.8))"
          : "none";
      });
